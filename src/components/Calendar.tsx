...
function EventTooltip({ event }: EventContentArg) {
  ...
  const end = event.end ? format(event.end, "MMM d, yyyy h:mm a") : null;
  const organizer =
    (event.extendedProps.organizer as typeof profiles.$inferSelect)?.name ??
    "Unknown";
  // const tags =
  //   event.extendedProps.tags?.tags.map((tag: string) => <p>{tag} </p>) ||
  //   "None";
          ...
          <button
            onClick={(e) => {
              ...
              downloadICS({
                ...
                organizerId: String(event.extendedProps.organizerId),
                tags: [],
              });
            }}
...
function EventContent({ event, ...tooltipProps }: EventContentArg) {
  return (
    <Tippy
      content={<EventTooltip event={event} {...tooltipProps} />}
      placement="auto"
      interactive={true}
      animation="fade"
      duration={200}
    ...
    >
      <div
        ...
        className="flex cursor-pointer flex-col gap-0.5 rounded-sm border-l-[3px] border-l-sky-500 bg-white/95 px-1.5 py-1 shadow-sm transition-all hover:-translate-y-px hover:shadow-md"
      >
